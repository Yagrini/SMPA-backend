...
public class PasswordResetService {
    ...
    private static String issuer = "smpa_platform";
    ...
    public static String resetPasswordURI(UriInfo uriInfo, String email){
        try {
            // generate a token for the user
            String token = PasswordResetService.generateJwtToken( email );

            // write the token to the database
            UserDAO userDAO = new UserDAO();
            userDAO.updateToken(email,token);

            return uriInfo.getBaseUriBuilder().path(AuthenticationResource.class).toString()+ "/reset-password/" +token;
        }
        catch( Exception e ) {
            return null;
        }
    ...
    public static String generateJwtToken( String email ) throws JoseException {
        // Give the JWK a Key ID (kid), which is just the polite thing to do
        rsaJsonWebKey.setKeyId("smpa");
    ...
    public static String validateJwtToken( String jwt ) throws InvalidJwtException {
        ...
        return jwtClaims.getClaimsMap().get("email").toString();
    }

    public static void main(String[] args) throws JoseException, InvalidJwtException {
        //System.out.println(rsaJsonWebKey.getKey());
        //System.out.println(rsaJsonWebKey.getPrivateKey().toString());
        System.out.println(PasswordResetService.generateJwtToken("you"));
    }
